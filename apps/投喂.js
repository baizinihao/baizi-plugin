...
export class ZanzhuPlugin extends plugin {
  ...
  async showZanzhu(e) {
    try{
      ...
      if(!img)return await e.reply('截图生成失败');
...
export class AddZanzhuPlugin extends plugin {
  ...
  async addZanzhu(e) {
    ...
    for(i=0;i<d.length;i++){if(d[i].qqnumber===q){d[i].money+=mo;f=true;break;}}
  ...
  async deleteZanzhu(e) {
    ...
    d=await this.getData();
    for(i=0;i<d.length;i++){if(d[i].qqnumber===q){f=true;}else{nd.push(d[i]);}}
    if(!f){return await e.reply('未找到该记录');}
